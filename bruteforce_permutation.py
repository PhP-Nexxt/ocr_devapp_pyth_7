...
def get_max_performance(shares, max_amount):
    ...
    max_result = None
    i = 0
    for permutation in permutations(performances):
        print(permutation)
        if permutation[0].price < 0: #Skip prix négatifs
            continue
        amount = 0
        result = 0
        performance = 0
        for share in permutation:
            ...
            if amount >= share.price and share.price > 0: # Verification du dataset (valeurs Négatives)
                result.append(share)
                amount -= share.price
